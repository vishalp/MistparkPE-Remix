...
function message_post(&$a) {
	...
	$contact = q("SELECT * FROM `contact` WHERE `id` = %d LIMIT 1",
			intval($recipient)
	);
	...
	$r = q("INSERT INTO `mail` ( `from-name`, `from-photo`, `from-url`, 
		...
		VALUES ( '%s', '%s', '%s', %d, '%s', '%s', %d, %d, %d, '%s', '%s', '%s' )",
		dbesc($me[0]['name']),
		dbesc($me[0]['thumb']),
		dbesc($me[0]['url']),
		intval($recipient),
	...
	$r = q("SELECT * FROM `mail` WHERE `uri` = '%s' LIMIT 1",
		dbesc($uri)
	);
	if(count($r))
		$post_id = $r[0]['id'];
	...
	if($post_id) {
		proc_close(proc_open("php include/notifier.php \"$url\" \"mail\" \"$post_id\" &",
			array(),$foo));
...
function message_content(&$a) {
	...
	if(($a->argc == 3) && ($a->argv[1] == 'drop' || $a->argv[1] == 'dropconv')) {
		...
		$cmd = $a->argv[1];
		if($cmd == 'drop') {
			$r = q("DELETE FROM `mail` WHERE `id` = %d LIMIT 1",
				intval($a->argv[2])
			);
			if($r) {
			...
			$r = q("SELECT `parent-uri` FROM `mail` WHERE `id` = %d LIMIT 1",
				intval($a->argv[2])
			);
			if(count($r)) {
				$parent = $r[0]['parent-uri'];
				$r = q("DELETE FROM `mail` WHERE `parent-uri` = '%s' ",
					dbesc($parent)
				);
				if($r)
	...
	if(($a->argc == 1) || ($a->argc == 2 && $a->argv[1] == 'sent')) {
		...
		$r = q("SELECT count(*) AS `total` FROM `mail` 
			WHERE `from-url` $eq '%s' GROUP BY `parent-uri` ORDER BY `created` DESC",
			dbesc($myprofile)
		);
		if(count($r))
		...
		$r = q("SELECT max(`mail`.`created`) AS `mailcreated`, min(`mail`.`seen`) AS `mailseen`, 
			...
			WHERE `from-url` $eq '%s' GROUP BY `parent-uri` ORDER BY `created` DESC  LIMIT %d , %d ",
			dbesc($myprofile),
	...
	if(($a->argc > 1) && (intval($a->argv[1]))) {
		...
		if(count($r)) { 
			...
			$messages = q("SELECT `mail`.*, `contact`.`name`, `contact`.`url`, `contact`.`thumb` 
				FROM `mail` LEFT JOIN `contact` ON `mail`.`contact-id` = `contact`.`id` 
				WHERE `mail`.`parent-uri` = '%s' ORDER BY `mail`.`created` ASC",
		...
		$r = q("UPDATE `mail` SET `seen` = 1 WHERE `parent-uri` = '%s' ",
			dbesc($r[0]['parent-uri'])
		);
