...
function dfrn_request_post(&$a) {
	...
	if((x($_POST,'localconfirm')) && ($_POST['localconfirm'] == 1)) {
		...
		if(local_user() && ($a->user['nickname'] == $a->argv[1]) && (x($_POST,'dfrn_url'))) {
			...
			if(x($dfrn_url)) {
				...
				$r = q("SELECT * FROM `contact` WHERE `url` = '%s' LIMIT 1",
					dbesc($dfrn_url)
				);
				...
				if(is_array($contact_record)) {
					...
					$r = q("INSERT INTO `contact` ( `created`,`url`, `name`, `photo`, `site-pubkey`,
	...
	if( x($_POST,'dfrn_url')) {
		...
		$ret = q("SELECT * FROM `contact` WHERE `url` = '%s' LIMIT 1", 
			dbesc($url)
		);
		...
		if(is_array($contact_record)) {
			...
			dbesc_array($parms);
			$r = q("INSERT INTO `contact` ( `created`, `url`, `name`, `issued-id`, `photo`, `site-pubkey`,
			...
			if($r) {	
				$r = q("SELECT `id` FROM `contact` 
					WHERE `url` = '%s' AND `issued-id` = '%s' LIMIT 1",
					$parms['url'],
					$parms['issued-id']
				);
				if(count($r)) 
		...
		if(is_array($contact_record)) {
			$ret = q("INSERT INTO `intro` ( `contact-id`, `blocked`, `knowyou`, `note`, `hash`, `datetime`)
				VALUES ( %d, 1, %d, '%s', '%s', '%s' )",
				intval($contact_record['id']),
