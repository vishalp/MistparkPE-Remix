...
function contacts_post(&$a) {
	...
	$orig_record = q("SELECT * FROM `contact` WHERE `id` = %d LIMIT 1",
		intval($contact_id)
	);
	...
	if($profile_id) {
		$r = q("SELECT `id` FROM `profile` WHERE `id` = %d LIMIT 1",
			intval($profile_id)
		);
		if(! count($r)) {
...
function contacts_content(&$a) {
	...
	if($a->argc == 3) {
		...
		if($cmd == 'block') {
			...
			$r = q("UPDATE `contact` SET `blocked` = %d WHERE `id` = %d LIMIT 1",
					intval($blocked),
					intval($contact_id)
			);
			if($r) {
	...
	$r = q("SELECT COUNT(*) AS `total` FROM `contact` 
		WHERE `pending` = 0 $sql_extra $sql_extra2 ");
	if(count($r))
