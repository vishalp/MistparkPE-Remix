...
function profiles_content(&$a) {
	...
	if(($a->argc > 2) && ($a->argv[1] == "drop") && intval($a->argv[2])) {
		$r = q("SELECT * FROM `profile` WHERE `id` = %d AND `is-default` = 0 AND `self` = 0 LIMIT 1",
			intval($a->argv[2])
		);
		if(! count($r)) {
			notice( t('Profile not found.') . EOL);
		...
		$r = q("UPDATE `contact` SET `profile-id` = (SELECT `profile`.`id` AS `profile-id` FROM `profile` WHERE `profile`.`is-default` = 1 LIMIT 1) WHERE `profile-id` = %d ",
			intval($a->argv[2])
		);
	...
	if(($a->argc > 1) && ($a->argv[1] == 'new')) {
		...
		$r3 = q("SELECT `id` FROM `profile` WHERE `profile-name` = '%s' LIMIT 1",
			dbesc($name)
		);
